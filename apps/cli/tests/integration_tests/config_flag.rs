...
pub fn test_multiple_configs_no_crosstalk() {
  ...
  "tmux": {
    ...
    let (stdout_a, _) = helpers::run_cli_with_stdin(
        &["workspace", "list"],
        "",
        test_ctx_a.config_path(),
    );
    assert!(stdout_a.contains("config_a"), "Config A should show config_a");
    assert!(!stdout_a.contains("config_b"), "Config A should not show config_b");
    assert!(!stdout_a.contains("config_c"), "Config A should not show config_c");

    // Test config B
    let test_ctx_b = helpers::TestContext::new(config_b).expect("Failed to create test config B");
    let (stdout_b, _) = helpers::run_cli_with_stdin(
        &["workspace", "list"],
        "",
        test_ctx_b.config_path(),
    );
    assert!(!stdout_b.contains("config_a"), "Config B should not show config_a");
    assert!(stdout_b.contains("config_b"), "Config B should show config_b");
    assert!(!stdout_b.contains("config_c"), "Config B should not show config_c");

    // Test config C
    let test_ctx_c = helpers::TestContext::new(config_c).expect("Failed to create test config C");
    let (stdout_c, _) = helpers::run_cli_with_stdin(
        &["workspace", "list"],
        "",
        test_ctx_c.config_path(),
    );
    assert!(!stdout_c.contains("config_a"), "Config C should not show config_a");
    assert!(!stdout_c.contains("config_b"), "Config C should not show config_b");
    assert!(stdout_c.contains("config_c"), "Config C should show config_c");
}
