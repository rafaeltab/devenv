...
///
/// DEPRECATED: Use `CliTestRunner::new().with_config(path).run(args)` instead.
#[allow(dead_code)]
pub fn run_cli(args: &[&str], config_path: &str) -> (String, String, bool) {
    CliTestRunner::new().with_config(config_path).run(args)
}

/// Helper function to run the CLI with tmux socket isolation.
/// Returns (stdout, stderr, success_status).
///
/// DEPRECATED: Use `CliTestRunner::new().with_config(path).with_tmux(socket).run(args)`
/// or `CliTestRunner::new().with_env(&env).run(args)` instead.
#[allow(dead_code)]
...
///
/// DEPRECATED: Use `CliTestRunner::new().with_config(path).with_tmux(socket).with_tui().run(args)`
/// or `CliTestRunner::new().with_env(&env).with_tui().run(args)` instead.
#[allow(dead_code)]
